...
printf '%s\n' \
  'MEIMAOBING_IMAGE_GATEWAY_PUBLIC_ORIGIN=https://image.truthtruth.co' \
  'MEIMAOBING_IMAGE_GATEWAY_COOKIE_SECURE=true' \
  'MEIMAOBING_IMAGE_GATEWAY_OIDC_ISSUER=https://auth.truthtruth.co' \
  'MEIMAOBING_IMAGE_GATEWAY_OIDC_CLIENT_ID=meimaobing-image-gateway-truthtruth-beta' \
  ...
  'MEIMAOBING_IMAGE_GATEWAY_TOP_UP_URL=https://store.truthtruth.co/user/recharge/index' >"$secrets_dir/image-gateway.env"
printf '%s\n' \
...
printf '%s\n' \
  ...
  'MEIMAOBING_IMAGE_GATEWAY_ENABLED=false' \
  'MEIMAOBING_IMAGE_GATEWAY_STORE_INGRESS_PROFILE=cutover-truthtruth-isolated' >"$compose_env"
...
sed -i.bak 's#https://image.truthtruth.co#https://wrong-beta.sg.aidenovak.com#' "$secrets_dir/image-gateway.env"
if PATH="$fake_bin:$PATH" "$script_dir/verify-beta.sh" "$compose_env" >/dev/null 2>&1; then
  printf '%s\n' 'verify-beta accepted a gateway origin other than Image Beta' >&2
  exit 1
fi

sed -i.bak 's#https://wrong-beta.sg.aidenovak.com#https://image.truthtruth.co#' "$secrets_dir/image-gateway.env"
sed -i.bak 's/MEIMAOBING_IMAGE_GATEWAY_OIDC_CLIENT_ID=meimaobing-image-gateway-truthtruth-beta/MEIMAOBING_IMAGE_GATEWAY_OIDC_CLIENT_ID=another-image-client/' "$secrets_dir/image-gateway.env"
if PATH="$fake_bin:$PATH" "$script_dir/verify-beta.sh" "$compose_env" >/dev/null 2>&1; then
  printf '%s\n' 'verify-beta accepted a gateway client other than Image Beta' >&2
  exit 1
fi

sed -i.bak 's/MEIMAOBING_IMAGE_GATEWAY_OIDC_CLIENT_ID=another-image-client/MEIMAOBING_IMAGE_GATEWAY_OIDC_CLIENT_ID=meimaobing-image-gateway-truthtruth-beta/' "$secrets_dir/image-gateway.env"
sed -i.bak 's#https://store.truthtruth.co/user/recharge/index#https://store.truthtruth.co/subscribe#' "$secrets_dir/image-gateway.env"
if PATH="$fake_bin:$PATH" "$script_dir/verify-beta.sh" "$compose_env" >/dev/null 2>&1; then
  printf '%s\n' 'verify-beta accepted the legacy subscription destination' >&2
  exit 1
fi

sed -i.bak 's#https://store.truthtruth.co/subscribe#https://store.truthtruth.co/user/recharge/index#' "$secrets_dir/image-gateway.env"
sed -i.bak 's/MEIMAOBING_IMAGE_GATEWAY_STORE_INGRESS_PROFILE=cutover-truthtruth-isolated/MEIMAOBING_IMAGE_GATEWAY_STORE_INGRESS_PROFILE=beta-sg-aidenovak/' "$compose_env"
if PATH="$fake_bin:$PATH" "$script_dir/verify-beta.sh" "$compose_env" >/dev/null 2>&1; then
  printf '%s\n' 'verify-beta accepted a non-Truthtruth Store profile' >&2
  exit 1
fi

sed -i.bak 's/MEIMAOBING_IMAGE_GATEWAY_STORE_INGRESS_PROFILE=beta-sg-aidenovak/MEIMAOBING_IMAGE_GATEWAY_STORE_INGRESS_PROFILE=cutover-truthtruth-isolated/' "$compose_env"
